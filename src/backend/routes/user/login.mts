...
router.get('/verify', (req, res) => {
	...
	if(typeof token !== 'string') return sendErrorPage(req, res, 'Unauthorized');
	const cloneTemplate = { ...loginTemplate };

	try {
		if(!process.env.JWT_SECRET) throw new Error('Missing JWT Secret');
		const decodedToken = jwt.verify(token, process.env.JWT_SECRET);
		if(typeof decodedToken !== 'object') throw new Error('Missing JWT user ID');

		const user = getUsers().find((thisUser) => thisUser.id === decodedToken.userId);
		if(typeof user === 'undefined') throw new Error(`Unkown User tried to login: ${JSON.stringify(decodedToken)}`);
