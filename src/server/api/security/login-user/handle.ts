...
import { NotFoundException } from '../../../modules/misc/custom-errors.js';
...
const loginUserHandle = (): Handler => {
	return async (req, res, next) => {
		try {
			...
			if (!user?.active) throw new NotFoundException('User not found or wrong credentials');

			const isValidPassword = await comparePassword(
				requestBody.password,
				user.password,
			);
			if (!isValidPassword) throw new NotFoundException('User not found or wrong credentials');
